...
const Navbar = () => {
    ...
    return (
        <nav className="fixed w-full z-50 glass border-b border-slate-100 top-0 transition-all duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    <div className="flex items-center">
                        <Link to="/" className="text-xl md:text-2xl font-extrabold gradient-text tracking-tighter">
                            SHOPKART
                        </Link>
                    </div>
            ...
            {/* Mobile Menu */}
            <div className={`${isOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0 pointer-events-none"} absolute top-20 left-0 w-full md:hidden glass-card rounded-none border-x-0 transition-all duration-300 z-40`}>
                <div className="px-4 pt-4 pb-6 space-y-2">
                    <Link to="/" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl">Home</Link>
                    <Link to="/shop" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl">Shop</Link>
                    <Link to="/cart" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl flex justify-between items-center">
                        Cart
                        <span className="bg-primary/10 text-primary px-2 py-0.5 rounded-lg text-xs">{cartItems.length}</span>
                    </Link>
                    {user ? (
                        <>
                            <div className="pt-4 border-t border-slate-100 mt-4 space-y-2">
                                <Link to="/profile" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl">My Account</Link>
                                {user.role === "admin" && (
                                    <Link to="/admin" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-primary font-black hover:bg-primary/5 rounded-xl">Admin Panel</Link>
                                )}
                                <button onClick={() => { logout(); setIsOpen(false); }} className="w-full text-left px-4 py-3 text-red-500 font-bold hover:bg-red-50 rounded-xl">Logout</button>
                            </div>
                        </>
                    ) : (
                        <Link to="/login" onClick={() => setIsOpen(false)} className="block px-4 py-3 text-primary font-black hover:bg-primary/5 rounded-xl text-center mt-4">Login</Link>
                    )}
                </div>
            </div>
        </nav>
